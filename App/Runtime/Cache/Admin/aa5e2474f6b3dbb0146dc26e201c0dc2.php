...
<body>
 <div class="navbox">
      <div class="taskbox">
        <h3>通知列表</h3>
        <?php if(is_array($notice)): foreach($notice as $key=>$t1): ?><strong><?php echo ($t1["title"]); ?></strong>&nbsp;&nbsp;<?php echo date('Y-m-d',$t1['time']);?>
...
<form id="addActivity" action="<?php echo U('Admin/Notice/noticeHandle');?>" method="post"  enctype="multipart/form-data">
    <table class="table table-bordered table-hover">
        <tr>
            <input type="hidden" name="id" value="" />
            <td width="10%" class="tableleft">标题</td>
            <td><input type="text" class="form-control" style="width:200px;" name="title" value="<?php echo ($notice["title"]); ?>"/></td>
        </tr>
         <tr>
            ...
            <td>
                <select id="select" name="task">
                 ...
                 <?php if(is_array($status)): foreach($status as $key=>$t1): ?><option  value="<?php echo ($t1['id']); ?>"><?php echo ($t1["name"]); ?></option><?php endforeach; endif; ?>
                </select>&nbsp;&nbsp;&nbsp;&nbsp;管理范围的阶段
            </td>
        </tr>
...
<script>
    $(document).ready(function(){
      ...
      $(".edit").click(function(){
        ...
        $.ajax({
             type: 'POST',
             url: '<?php echo U("Admin/Notice/editNotice");?>' ,
     ...
     var task_id = $(this).attr("taskid");
         var d = dialog({
              ...
              ok: function () {
                 $.ajax({
             type: 'POST',
             url: '<?php echo U("Admin/Notice/delnoticeHandle");?>' ,
             data: {task_id:task_id} ,
             dataType: 'json',
             success:function(data){
               if(data){
